...
async function comments_init(thread) {
	...
	relay.event = (sub_id, ev) => {
		if (sub_id === comments_id) {
			if (ev.content !== '') insert_event_sorted(model.events, ev);
...
function render_event(model, ev) {
	...
	if (parsed) {
		...
		ev.created_at > 1678191250 &&
		ev.pubkey !== 'd91191e30e00444b942c0e82cad470b32af171764c2275bee0bd99377efd4075'
	) {
...
function fundingEvent(parsed, profile, ev) {
	try {
		if (
			verifyBitcoinSignedEvent(window.NostrTools.nip19.npubEncode(ev.pubkey), parsed[0], parsed[1])
		) {
...
function joinOrderEvent(parsed, profile, ev) {
	...
	try {
		if (npubList.indexOf(ev.pubkey) != -1) {
			...
			let link = document.createElement('a');
			link.href = 'https://nostr.band/?q=' + window.NostrTools.nip19.npubEncode(ev.pubkey);
			link.innerText = name;
